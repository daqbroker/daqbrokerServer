...
from daqbrokerServer.web.routes.utils import get_current_user, Depends, HTTPException, oauth2_scheme, AuthUser
...
from daqbrokerServer.web.routes.utils import get_user, get_connection, get_db
...
async def add_user_connection(conn_ids: List[int], username: str, current_user: User = Depends(get_current_user), session= Depends(get_db)):
	user = get_user(session, username, HTTP_404_NOT_FOUND, err_msg = "User '" + username + "' not found")
...
async def remove_user(username: str, conn_ids: List[int], current_user: User = Depends(get_current_user), session= Depends(get_db)):
	user = get_user(session, username, HTTP_404_NOT_FOUND, err_msg = "User '" + username + "' not found")
	to_delete = []
	for conn_id in conn_ids:
		connection  = get_connection(session, conn_id, HTTP_404_NOT_FOUND, err_msg="Connection with id='" + str(conn_id) + "' not found")
